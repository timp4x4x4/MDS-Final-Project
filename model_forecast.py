...
def load_california_data(file_path):
    """載入只包含 California 的資料"""
    print(f"\n載入資料: {file_path}")
...
def split_data_by_year(df):
    """按年份分割資料"""
    # 訓練資料：2016-2022
    df_train = df[df['Year'].between(2016, 2022)].copy()
    # 測試資料：2023
    df_test = df[df['Year'] == 2023].copy()
    
    print(f"\n訓練資料 (2016-2022): {len(df_train):,} 筆")
    print(f"測試資料 (2023): {len(df_test):,} 筆")
    
    # 顯示訓練資料的目標變數分布
    print("\n訓練資料目標變數分布:")
    train_severity = df_train['Severity'].value_counts().sort_index()
    for sev, count in train_severity.items():
        print(f"  Severity {sev}: {count:,} ({count/len(df_train)*100:.2f}%)")
...
def advanced_preprocessing(df):
    ...
    df_copy['IsRushHour'] = df_copy['Hour'].apply(
        lambda x: 1 if (6 <= x <= 9) or (15 <= x <= 19) else 0
    )
    ...
    for col in numeric_cols:
        if col not in ['Severity', 'Year']:
            df_copy[col] = df_copy[col].fillna(df_copy[col].median())
    
    # 7. 類別變數編碼
    label_encoders = {}
    categorical_cols = ['Sunrise_Sunset', 'City', 'County']
    ...
    # 刪除不需要的欄位
    df_copy = df_copy.drop(['Start_Time', 'State', 'Year'], axis=1, errors='ignore')
...
def run_california_experiment(df_train, df_test):
    """執行 California 專用實驗"""
    results = {}
...
def create_kepler_predictions_2023(df_test, model, feature_columns, label_encoders):
    """使用 2023 年的實際位置進行預測"""
    print("\n準備 2023 年 Kepler.gl 預測資料...")
    
    # 處理 2023 年的資料
    df_test_processed, _ = advanced_preprocessing(df_test)
    ...
    output_df = pd.DataFrame({
        'lat': df_test['Start_Lat'].values,
        'lng': df_test['Start_Lng'].values,
        'timestamp': df_test['Start_Time'].values,
        'actual_severity': df_test['Severity'].values,
    ...
    # 估計每筆資料約 200 bytes，目標 200MB = 1M 筆資料
...
def main():
    ...
    df_ca = load_california_data(file_path)
    
    # 按年份分割
    df_train, df_test = split_data_by_year(df_ca)
    
    # 執行實驗
    results, df_test = run_california_experiment(df_train, df_test)
    
    # 產生 Kepler.gl 預測資料
    kepler_df = create_kepler_predictions_2023(
        df_test, 
        results['model'], 
    ...
    # 儲存結果
    output_file = 'california_accidents_2023_predictions.csv'
    ...
    if 'actual_severity' in kepler_df.columns:
        accuracy = (kepler_df['actual_severity'] == kepler_df['predicted_severity']).mean()
        print(f"\n2023年資料預測準確率: {accuracy:.4f}")
